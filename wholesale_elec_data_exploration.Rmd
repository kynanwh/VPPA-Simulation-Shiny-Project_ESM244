...
caiso_iso_hr2 <- caiso_iso_5min %>% 
  ...
  group_by(date,hour) %>% 
  dplyr::summarise(m = mean(price))

#transform 5-minute interval data to hourly by taking hourly averages. Note: transforms date/time into factor
...
caiso_tsibble <- caiso_iso_5min %>%
  as_tsibble(key = id(price), index = Date)
